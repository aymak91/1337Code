...
class LinkedListNode {
	...
	reverse(): LinkedListNode {
		let current: LinkedListNode | null = this;
		let prev: LinkedListNode | null = null;
		while (current !== null) {
			const next = current.next as LinkedListNode;
			current.next = prev;
			prev = current;
			current = next;
		}
...
export const reverseLinkedListHandler = (fn: any) => {
	try {
		...
		for (let i = 0; i < tests.length; i++) {
			const list = createLinkedListNode(tests[i]);
...
// returns LL type
function createLinkedListNode(values: number[]): LinkedListNode {
...
// returns number array type
function getListValues(head: LinkedListNode): number[] {
	const values: number[] = [];
